...
class TextDetector:
    ...
    @torch.no_grad()
    def __call__(self, img, refine_mode=REFINEMASK_ANNOTATION, keep_undetected_mask=False, bgr2rgb=True):
...
DEFAULT_MODEL = None
def load_model(cuda: bool, path: str, path_cpu):
    ...
    if cuda:
        ...
        model = TextDetector(model_path=path_cpu, device=device, act='leaky', input_size=1024)
    DEFAULT_MODEL = model

async def dispatch(img: np.ndarray, cuda: bool, modelpath: str, modelpath_onnx: str):
    global DEFAULT_MODEL
    if DEFAULT_MODEL is None :
        load_model(cuda, modelpath, modelpath_onnx)
    return DEFAULT_MODEL(img, refine_mode=REFINEMASK_ANNOTATION, keep_undetected_mask=False, bgr2rgb=False)